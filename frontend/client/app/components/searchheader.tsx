...
export function useAutocomplete(initialQuery = "") {
  ...
  return { query, setQuery, suggestions, richSuggestions, showSuggestions, setShowSuggestions };
}
...
function SearchHeaderContent() {
  ...
  const handleSearch = (e?: React.FormEvent, overrideQuery?: string) => {
    ...
    if (q.trim()) {
      setIsLoading(true);
      setShowSuggestions(false);
      router.push(`/search/${activeTab}?q=${encodeURIComponent(q)}&safesearch=on`);
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between mb-4 md:mb-0 md:absolute md:inset-x-0 md:top-6 md:px-8 pointer-events-none">
          <Link href="/" className="shrink-0 pointer-events-auto">
            <Image src="/images/pyxis.svg" alt="Pyxis" width={90} height={30} className="w-[80px] h-auto md:w-[110px]" priority />
          </Link>
          <Link href="/signin" className="px-4 py-1.5 md:px-5 md:py-2.5 bg-black text-white rounded-full text-sm font-medium hover:opacity-90 transition-opacity pointer-events-auto">
            Sign In
          </Link>
        </div>
        ...
        <div className="flex pb-4">
          <div ref={containerRef} className="w-full md:max-w-2xl relative md:mt-1">
            <form onSubmit={handleSearch} className="relative w-full text-gray-500 focus-within:text-black">
              <input
                type="text" readOnly value={ghostText} aria-hidden="true"
              ...
              />
              <input
                type="text" id="search-input-header" name="q" value={query}
              ...
              />
              <button type="submit" className="absolute right-0 top-0 h-full w-12 flex items-center justify-center text-black hover:opacity-70 transition-opacity z-20">
                  ...
                  {suggestions.map((s, i) => (
                    <div key={i} onClick={() => { setQuery(s); handleSearch(undefined, s); }}
                      ...
                      <span className="text-black text-sm md:text-base">
                        <span className="font-semibold">{s.substring(0, query.length)}</span>
      ...
      <div className="w-full max-w-[1200px] mx-auto px-4 md:px-8">
        <div className="flex items-center gap-6 overflow-x-auto no-scrollbar">
          {tabs.map((tab) => {
            ...
            return (
              <Link key={tab.path}
                href={query.trim() ? `/search/${tab.path}?q=${encodeURIComponent(query)}&safesearch=on` : '#'}
                onClick={(e) => {
              ...
              >
                <span className={`flex items-center gap-2 pb-[1px] border-b-2 ${isActive ? "border-black" : "border-transparent"} ${!query.trim() && !isActive ? "opacity-40 cursor-not-allowed" : ""}`}>
