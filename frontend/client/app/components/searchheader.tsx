  ...
  return (
    <header className="sticky top-0 bg-white z-50 pt-4 md:pt-6 border-b border-gray-200 transition-colors">
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        ...
        {/* Search Bar Row responsiveness for both mobile and desktop */}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                onClick={() => { if (!isActive) setIsLoading(true); }}
              ...
              >
                <span className={`flex items-center gap-2 pb-[1px] border-b-2 ${isActive ? 'border-black' : 'border-transparent'}`}>
                  <span className={isActive ? 'opacity-100' : 'opacity-70 group-hover:opacity-100'}>{tab.icon}</span>
                  <span>{tab.name}</span>
                </span>
              </Link>
            );
